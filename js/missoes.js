const missionsDataMock = [
  {
    id: 1,
    title: "Primeira Venda do Dia",
    description:
      "Realize sua primeira venda do dia para começar com o pé direito",
    icon: "🎯",
    type: "daily",
    status: "completed",
    current: 1,
    ...
    requirements: ["Realizar 1 venda hoje"],
  },
  {
    id: 2,
    ...
    progress: 70,
    points: 100,
    requirements: ["Realizar 10 vendas em um único dia"],
  },
  {
    id: 3,
  ...
  },
  {
    ...
    progress: 75,
    points: 80,
    requirements: ["Realizar 20 ligações telefônicas"],
  },
  {
    id: 5,
  ...
  },
  {
    ...
    progress: 71,
    points: 200,
    requirements: ["Vender por 7 dias seguidos", "Não pode perder nenhum dia"],
  },
  {
    id: 7,
  ...
  },
  {
    ...
    requirements: ["Participar de 5 reuniões", "Ser pontual em todas"],
  },
  {
    id: 9,
...
let currentMissions = [...missionsDataMock];
...
function createMissionCard(mission) {
  ...
  const statusClass =
    mission.status === "completed" ? "status-completed" : "status-in-progress";
  const statusText =
  ...
  card.innerHTML = `
        ...
        <div class="mission-progress-section">
            <div class="mission-progress-bar">
                <div class="mission-progress-fill" style="width: ${
                  mission.progress
                }%"></div>
            </div>
            <div class="mission-progress-text">
                <span class="progress-count">${mission.current} de ${
    mission.target
  }</span>
...
function openModal(mission) {
  ...
  if (mission.status === "completed") {
    ...
    modalCompletionDate.textContent = `Concluída em ${formatDate(
      mission.completionDate
    )}`;
...
function updateStats() {
  const completed = missionsDataMock.filter(
    (m) => m.status === "completed"
  ).length;
  const active = missionsDataMock.filter(
    (m) => m.status === "in-progress"
  ).length;
  const total = missionsDataMock.length;
  const totalPoints = missionsDataMock
  ...
  if (totalPointsElement)
    animateCounter(totalPointsElement, 0, totalPoints, 1000);
  ...
  if (completedMissionsElement)
    animateCounter(completedMissionsElement, 0, completed, 800);
  if (activeMissionsElement)
    animateCounter(activeMissionsElement, 0, active, 800);
}
...
function getMissionsByType(type) {
  return missionsDataMock.filter((mission) => mission.type === type);
}

function getMissionsByStatus(status) {
  return missionsDataMock.filter((mission) => mission.status === status);
}

function getTotalPointsEarned() {
  return missionsDataMock
...
function completeMission(missionId) {
  const mission = missionsDataMock.find((m) => m.id === missionId);
  ...
  showNotification(
    `Missão "${mission.title}" concluída! +${mission.points} pontos`,
    "success"
  );
  updateStats();
  renderMissions();
