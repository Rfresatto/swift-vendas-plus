const achievementsDataMock = [
  ...
  },
  {
    ...
    name: "Top Vendedor",
    description:
      "Fique entre os 3 melhores vendedores por 3 meses consecutivos",
    icon: "🏆",
    points: 400,
    unlocked: false,
    progress: 67,
    requirements: [
      "Ficar no top 3 por 3 meses consecutivos",
      "Atualmente: 2/3 meses",
    ],
    category: "lideranca",
    rarity: "epico",
  },
...
let currentAchievements = [...achievementsDataMock];
...
function filterAchievements(searchTerm = "") {
  let filtered = [...achievementsDataMock];
...
function createAchievementCard(achievement) {
  ...
  const statusClass = achievement.unlocked
    ? "status-unlocked"
    : "status-locked";
  ...
  card.innerHTML = `
        <div class="achievement-header">
            ...
            <div class="achievement-status ${
              isRecent ? "status-recent" : statusClass
            }">
...
function openModal(achievement) {
  ...
  modalReward.querySelector(
    ".reward-points"
  ).textContent = `+${achievement.points} pontos`;
...
function updateStats() {
  const unlockedCount = achievementsDataMock.filter((a) => a.unlocked).length;
  const totalCount = achievementsDataMock.length;
...
function getAchievementsByCategory(category) {
  return achievementsDataMock.filter(
    (achievement) => achievement.category === category
  );
}

function getUnlockedAchievements() {
  return achievementsDataMock.filter((achievement) => achievement.unlocked);
}
