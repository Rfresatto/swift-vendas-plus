...
function setupEventListeners() {
  ...
  periodSelect?.addEventListener("change", (e) => {
    ...
    showNotification(
      `Período alterado para: ${e.target.options[e.target.selectedIndex].text}`,
      "info"
    );
  });
...
function renderRanking() {
  const data = rankingsDataMock[currentRanking];
  ...
  document.getElementById("rankingTotal").textContent = `${data.length} ${
    currentRanking === "loja" ? "vendedores" : "participantes"
  }`;
...
function renderPodium(topThree) {
  ...
  podium1.querySelector(
    ".podium-points"
  ).textContent = `${topThree[0].points.toLocaleString()} pts`;
  ...
  podium2.querySelector(
    ".podium-points"
  ).textContent = `${topThree[1].points.toLocaleString()} pts`;
  ...
  podium3.querySelector(
    ".podium-points"
  ).textContent = `${topThree[2].points.toLocaleString()} pts`;
}
...
function createRankingItem(person, index) {
  ...
  item.innerHTML = `
        <div class="rank-position ${isTop3 ? "top" : ""}">${
    person.position
  }º</div>
...
function openModal(person) {
  ...
  modalSubtitle.textContent = `${person.department}${
    person.location ? ` • ${person.location}` : ""
  }`;
  ...
  if (person.achievements.length === 0) {
    modalAchievements.innerHTML =
      '<p style="color: #666; font-size: 14px;">Nenhuma conquista ainda</p>';
  }
...
function getRankingByType(type) {
  return rankingsDataMock[type] || [];
}

function getUserPosition(type = currentRanking) {
  const data = rankingsDataMock[type];
